...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
	...
	useEffect(() => {
		...
		if (storedAuthState) {
			...
			if (storedAuthState === AuthState.OAUTH) {
				...
				setProof(proof ?? undefined);
				setSeed(seed ?? undefined)
			}
		}
		// console.log("session is", currentPage)
		// setCurrentPage(currentPage ? currentPage: "Record");
	}, []);

	const logout = () => {
		const cp = sessionStorage.getItem('currentPage')
		sessionStorage.clear();
		sessionStorage.setItem('currentPage', cp ? cp : "Record")
	...
	useEffect(() => {
		...
		if (authState === AuthState.OAUTH) {
			...
			sessionStorage.setItem('proof', proof ?? '');
			sessionStorage.setItem('seed', seed ?? '');
		}
		console.log("sesh is", sessionStorage.getItem('currentPage'))
	}, [authState, jwt, userSpecificData, zkLoginSignature, zkLoginAddress, ephemeralSecretKey, walletAccount, currentPage, proof, seed]);
